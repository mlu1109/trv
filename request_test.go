...
func TestRequest(t *testing.T) {
	...
	t.Run("TestMarshalRequestWithMutlipleQueries", func(t *testing.T) {
		// Given
		authenticationKey := "yourauthenticationkey"
		sseURL := true
		objectType := "TrainAnnouncement"
	...
	<QUERY sseurl="%t" objecttype="%s" orderby="%s" schemaversion="%s">
		...
		<INCLUDE>%s</INCLUDE>
			...
			authenticationKey,
			// Query 1
			sseURL, objectType, orderBy, schemaVersion, filterEQName, filterEQValue, filterGTEName, filterGTEValue, filterLTName, filterLTValue, filterExistsName, filterExistsValue, include[0], include[1], include[2], include[3], include[4],
			// Query 2
			sseURL, objectType, orderBy, schemaVersion, filterEQName, filterEQValue, filterGTEName, filterGTEValue, filterLTName, filterLTValue, filterExistsName, filterExistsValue, include[0], include[1], include[2], include[3], include[4])
		assert.Nil(t, err)
		assert.Equal(t, expected, actual)
